...
class ENet(chainer.Chain):
    def __init__(self, n_class):
        ch_init, ch1, ch2, ch3, ch4, ch5 = 16, 64, 128, 128, 64, 16
...
class InitialBlock(chainer.Chain):
    ...
    def __call__(self, x):
        h = F.concat((F.max_pooling_2d(x, 2), self.conv(x)))
...
class BottleneckModule(chainer.Chain):
    def __init__(self, in_ch, out_ch, dilate=1,
        ...
        if downsample:
            cbpr1 = CBPrelu(in_ch, bottleneck_ch, 2, stride=2)
        elif upsample:
            cbpr1 = CUpBPrelu(in_ch, bottleneck_ch, 1, upscale=2)
        else:
            cbpr1 = CBPrelu(in_ch, bottleneck_ch, 1)
        ksize2 = 5 if asymmetric else 3
        pad2 = 2 if asymmetric else 1
        super(BottleneckModule, self).__init__(
            cbpr1=cbpr1,
            cbpr2=CBPrelu(bottleneck_ch, bottleneck_ch, ksize2, pad=pad2,
                          dilate=dilate, asymmetric=asymmetric),
            cb3=ConvBn(bottleneck_ch, out_ch, 1),
            prelu=L.PReLU())
    ...
    def __call__(self, x):
        ...
        if self.upsample:
            x = self.upconv(x)
        return self.prelu(x + h)


class CBPrelu(chainer.Chain):
    '''
    This is a composite link of sequence of Convolution2D, BatchNormalization
    and PReLU. Note that the default value of nobias is True.
...
class CUpBPrelu(chainer.Chain):
    ...
    def __call__(self, x):
        return self.prelu(self.bn(self.upconv(x)))
